		...
		)
{
	...
	currentBasicBlock->node_id = 10;
		...
		printf(" [Monitor Thread Info]: %d, tid: %u, pid: %u, signature_queue_size: %u, function_stack_number: %d, static_stack_size: %d\n", ++monitor_process_num, (int)tid, (int)pid, mon_signature_queue_size, function_stack_number, static_show_stack_size(function_stack_number));
#endif
...
void monitor_thread_generator()
{
	...
	static unsigned int monitor_thread_generator_number = 2;
	...
	// staic way
	static_copy_stack(1, monitor_thread_generator_number);
	mon_dat->function_stack_number = monitor_thread_generator_number++; 
